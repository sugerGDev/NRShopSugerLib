...
- (instancetype)initWithBrowseItemArray:(NSArray *)browseItemArray currentIndex:(NSInteger)currentIndex
{
    ...
    if(self)
    {
        ...
        _isEqualRatio = YES;
        _isFirstOpen = YES;
        _screenWidth = [UIScreen mainScreen].bounds.size.width;//[UIScreen mainScreen].bounds.size.width;
...
- (void)initData
{
    for (MSSBrowseModel *browseItem in _browseItemArray)
    {
        ...
        if(_isEqualRatio)
        {
            ...
            {
                verticalRect = [browseItem.smallImageView.image mss_getBigImageRectSizeWithScreenWidth:[UIScreen mainScreen].bounds.size.width screenHeight:[UIScreen mainScreen].bounds.size.height];
                horizontalRect = [browseItem.smallImageView.image mss_getBigImageRectSizeWithScreenWidth:[UIScreen mainScreen].bounds.size.height screenHeight:[UIScreen mainScreen].bounds.size.width];
            }
        }
...
- (void)tap:(MSSBrowseCollectionViewCell *)browseCell
{
    ...
    if(browseItem.smallImageView)
    {
        ...
        if(_currentOrientation == UIDeviceOrientationLandscapeLeft)
        {
            ...
            rect = CGRectMake(rect.origin.y, [UIScreen mainScreen].bounds.size.width - rect.size.width - rect.origin.x, rect.size.height, rect.size.width);
        }
        else if(_currentOrientation == UIDeviceOrientationLandscapeRight)
        {
            transform = CGAffineTransformMakeRotation(M_PI / 2);
            rect = CGRectMake([UIScreen mainScreen].bounds.size.height - rect.size.height - rect.origin.y, rect.origin.x, rect.size.height, rect.size.width);
        }
