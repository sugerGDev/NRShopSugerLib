...
@interface KYGooeyMenu()<CAAnimationDelegate>
...
-(id)initWithOrigin:(CGPoint)origin andDiameter:(CGFloat)diameter andDelegate:(UIView *)controller themeColor:(UIColor *)themeColor {
...
//平分之后的角度,弧度制，因为sinf、cosf需要弧度制 //不做平分（原来代码）
//    CGFloat degree = (180/(menuCount+1))*(M_PI/180);
    
    CGFloat degree = (120/(menuCount+1) + 50.f)*(M_PI/180);
    ...
    for (int i = 0; i < menuCount; i++) {
        ...
        UIImageView *menuImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, imageWidth + 13, imageWidth  + 13)];
...
//        [verticalLineLayer addAnimation:morph_right forKey:@"bounce_0_1_right"];
        ...
        _isOpened = YES;
        [self.menuDelegate tapToSwitchOpen];
...
//            [verticalLineLayer addAnimation:morph_left forKey:@"bounce_1_0_right"];
        ...
        [self.menuDelegate tapToSwitchClose];
        _isOpened = NO;
